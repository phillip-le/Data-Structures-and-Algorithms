...
public class StrongHeapTest {
    ...
    public void testAllLeft() {
        assertFalse(StrongHeap.isStrongHeap(
                left(left(leaf(5), 90), 100)
        ));
    }

    @Test
    public void testGreatGrandchildren() {
        assertTrue(StrongHeap.isStrongHeap(
                tree(tree(left(leaf(1), 2), 5, leaf(3)), 10, tree(leaf(2), 6, leaf(3)))
        ));
    }

    @Test public void testTwoGreatGrandchildren() {
        assertFalse(StrongHeap.isStrongHeap(
                tree(tree(tree(leaf(11), 20, leaf(15)), 50, tree(leaf(15), 30, leaf(20))),
                        100, tree(leaf(30), 60, leaf(40)))
        ));
        assertFalse(StrongHeap.isStrongHeap(
                tree(tree(tree(leaf(11), 20, leaf(15)), 50, tree(leaf(15), 30, leaf(20))),
                        100, tree(leaf(30), 60, leaf(39)))
        ));
    }
}